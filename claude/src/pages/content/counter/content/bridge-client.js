(() => {
	...
	class BridgeClient {
		constructor() {
			...
			this._conversationRequests = new Map();
		...
		async requestConversation(orgId, conversationId) {
			const ready = await injectBridgeOnce();
			if (!ready) throw new Error('Conversation bridge unavailable');
			const key = `${orgId}:${conversationId}`;
			const existing = this._conversationRequests.get(key);
			if (existing) return existing;

			const request = this.request('conversation', { orgId, conversationId }, { timeoutMs: 20000 })
				.finally(() => {
					if (this._conversationRequests.get(key) === request) this._conversationRequests.delete(key);
				});
			this._conversationRequests.set(key, request);
			return request;
		}
		...
		invalidateConversation(conversationId) {
			...
			for (const key of this._conversationRequests.keys()) {
				if (key.endsWith(`:${conversationId}`)) this._conversationRequests.delete(key);
			}
		}
	...
	function injectBridgeOnce() {
		...
		bridgeReadyPromise = new Promise((resolve) => {
			...
			script.src = runtime.getURL('counter/injected/bridge.js');
