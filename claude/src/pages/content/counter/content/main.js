(() => {
	...
	let latestConversationSequence = -1;
	let hasConversationMetrics = false;
	const CONVERSATION_RETRY_DELAYS_MS = [2000, 5000, 15000, 30000];
	let conversationRetryTimer = null;
	let conversationRetryAttempt = 0;
	...
	async function refreshConversation() {
		...
		try {
			...
			ui.setConversationUnavailable({ stale: hasConversationMetrics });
	...
	function scheduleConversationRetry() {
		if (conversationRetryTimer || !currentConversationId) return;
		const expectedConversationId = currentConversationId;
		const delay = CONVERSATION_RETRY_DELAYS_MS[
			Math.min(conversationRetryAttempt, CONVERSATION_RETRY_DELAYS_MS.length - 1)
		];
		conversationRetryTimer = setTimeout(() => {
			conversationRetryTimer = null;
			if (currentConversationId !== expectedConversationId) return;
			conversationRetryAttempt = Math.min(
				conversationRetryAttempt + 1,
				CONVERSATION_RETRY_DELAYS_MS.length - 1
			);
	...
	async function handleConversationPayload({ orgId, conversationId, requestSequence, data }) {
		if (!conversationId || conversationId !== currentConversationId) return;
		if (typeof requestSequence === 'number') {
			if (requestSequence < latestConversationSequence) return;
			latestConversationSequence = requestSequence;
		}
		updateOrgIdIfNeeded(orgId);
		if (!data) return;

		const expectedConversationId = conversationId;
		const metrics = await CC.tokens.computeConversationMetrics(data, conversationId);
		if (currentConversationId !== expectedConversationId) return;
		clearConversationRetry();
		hasConversationMetrics = typeof metrics.totalTokens === 'number';
		ui.setConversationMetrics(metrics);
	}

	function handleConversationError({ conversationId } = {}) {
		if (!conversationId || conversationId !== currentConversationId) return;
		ui.setConversationUnavailable({ stale: hasConversationMetrics });
		scheduleConversationRetry();
	}
	...
	CC.bridge.on('cc:conversation_error', handleConversationError);
	CC.bridge.on('cc:message_limit', handleMessageLimit);

	async function handleUrlChange() {
		clearConversationRetry();
		currentConversationId = getConversationId();
		latestConversationSequence = -1;
		hasConversationMetrics = false;
