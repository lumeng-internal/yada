...
10. Hover 任一格，Tooltip 必须只有一行：`9月18日 周四 20:00 使用12次`；不得显示模型、释放说明或第二行。
...
本轮只使用 `ChatGPT-Yada-v4.1.0-official-only-UNVERIFIED.zip`，不要生成正式 Release 包，不要 merge main。
