...
轻量 Chrome / Edge MV3 扩展，当前版本 v2.2.1。只包含「复制全部 + 对话导航 + 提示词库」，保留 Vite + TypeScript，无新增依赖。
...
- **点击跳转**：可用的官方 TOC 按钮优先原生 click；已加载骨架按容器 ID 定位，扣除实际 Header 遮挡与 16px 留白。超过 600px 直接滚动，短距离约 280ms 缓动。200/600/1200/2000ms 重新查询同一 ID，偏差超过 40px 校正。未加载轮次先触发原生历史分页，目标 ID 出现后再跳转。原生补全失败时，每个会话每个标签页最多一次空 `?message=` 刷新兜底。滚轮、触摸、滚动键和会话切换取消后续校正。状态为「定位中 / 定位成功 / 该轮暂时无法定位」，补全中显示「正在加载更早记录 n/N」。
...
下载 `ChatGPT-Yada-v2.2.1-dist_chrome.zip`，解压并在 Chrome / Edge 扩展管理页开启开发者模式，加载 `dist_chrome`。本机目录：
...
本轮按 Owner 明确指定以 PATCH 发布 2.2.1：修复刷新后把局部骨架当成完整导航的缺陷，执行 `npm run release:patch`。只修改 gpt/，普通 push main，不 force push、不建 PR、不运行 CI 或 GitHub Actions。`HOSTED_CI = DISABLED_BY_OWNER_NO_QUOTA`（`NOT_USED_BY_POLICY`）。
