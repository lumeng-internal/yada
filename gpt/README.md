...
ChatGPT Yada 4.1.2 是一个轻量 Chrome MV3 扩展。它通过补齐 ChatGPT 自己的历史加载，恢复并保留官方长对话 Prompt Navigator，同时不移动读者的当前位置。

4.1.2 保持 Navigator、三环、热力图、复制全部和提示词的产品行为，把运行时改成按需读取：工具栏先出现，新回答只读最近增量，历史核对不再每 10 分钟全量扫描。
...
1. 加载 `gpt/dist_chrome`，或解压 `ChatGPT-Yada-v4.1.2-official-only-UNVERIFIED.zip` 后加载其中的 `dist_chrome`。
2. 刷新 ChatGPT 标签页。
...
旧测试包保留，新的 4.1.2 包不覆盖 4.0.3、4.0.4、4.1.0、4.1.1。打包会检查 package、lockfile、两份 manifest 与 ZIP 版本一致。
