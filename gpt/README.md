...
轻量 Chrome / Edge MV3 扩展，当前版本 v3.1.0。ChatGPT Yada 3.1.0 是单一扩展。安装一次即可获得：
...
- **自动准备官方导航**：在符合条件的桌面对话页上，扩大当前会话首次历史请求，并临时暴露 ChatGPT 自己的分页触发点，让官方导航完整出现。不创建 Yada 导航条，不滚动页面，不点击官方按钮。准备中显示 `导航准备中 36/128`；就绪后约 1.5 秒隐藏；未完整时保留并在 title 中说明原因。
...
1. 下载 `ChatGPT-Yada-v3.1.0-dist_chrome.zip`，解压并在 Chrome / Edge 扩展管理页开启开发者模式，加载 `dist_chrome`。
...
本轮按 MINOR 发布 3.1.0：把官方导航准备做进单一扩展。只修改 gpt/，推送到测试分支 `codex/gpt-native-navigation-v3`，不 push main、不 force push、不建 PR、不运行 CI 或 GitHub Actions。`HOSTED_CI = DISABLED_BY_OWNER_NO_QUOTA`（`NOT_USED_BY_POLICY`）。
