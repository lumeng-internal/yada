...
MAIN-world bridge 只传递 conversation id、消息 id、角色、cursor 和必要拓扑统计，不传正文。`boosted` 只存在于当前 tab 的临时状态，不写入 `chrome.storage`。`chrome.storage.local` 只保存提示词、额度事件元数据和额度历史缓存；不保存提问/回复正文、Cookie、Token、Authorization 或完整 API payload。不接入外部服务。
