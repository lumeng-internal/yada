...
ChatGPT Yada 4.0.0 是一个轻量 Chrome MV3 扩展。它通过补齐 ChatGPT 自己的历史加载，恢复并保留官方长对话 Prompt Navigator，同时不移动读者的当前位置。
...
- **提示词**：本地收藏、编辑、删除与复制。
- **Pro 额度**：沿用 Vibe Bar 规则，以最近 7 天 Chat 历史、本地事件和 `model_limits` 渐进估算。它不是 OpenAI 官方剩余数字。
...
1. 加载 `gpt/dist_chrome`，或解压 `ChatGPT-Yada-v4.0.0-official-only-UNVERIFIED.zip` 后加载其中的 `dist_chrome`。
2. 刷新 ChatGPT 标签页。
