...
"use strict";
(() => {
  // src/rail/virtualizerBridgePage.ts
  ...
  (() => {
    ...
    const hasHint = (object) => VIRTUALIZER_HINTS.some((key) => {
    ...
    const getVirtualizerScore = (object) => {
      ...
      const hintScore = VIRTUALIZER_HINTS.reduce((score, key) => {
        const value = safeGet(object, key);
        return score + (typeof value === "function" || value !== void 0 ? 1 : 0);
      }, 0);
    ...
    const isReactPropertyKey = (key) => typeof key === "string" && REACT_PROPERTY_PREFIXES.some((prefix) => key.startsWith(prefix));
    const getReactObjectsFromElement = (element) => {
      if (!(element instanceof Element)) {
        return [];
      }
      return getOwnKeys(element).filter(isReactPropertyKey).map((key) => safeGet(element, key)).filter(isObjectLike);
    };
    ...
    const shouldSkipGenericKey = (key) => {
      ...
      return text === "__proto__" || text === "constructor" || text === "prototype" || text === "ownerDocument" || text === "parentNode" || text === "children" || text === "childNodes" || text === "firstChild" || text === "lastChild" || text === "nextSibling" || text === "previousSibling" || text === "style" || text === "classList";
    };
    const isRelevantKey = (key) => {
      const text = (typeof key === "symbol" ? key.description || "" : String(key)).toLowerCase();
      return text.includes("virtual") || text.includes("scroll") || text.includes("list") || text.includes("range") || text.includes("index") || text.includes("item") || text.includes("ref") || text.includes("state") || text.includes("props");
    };
    ...
    const findVirtualizerApi = () => {
      const now = Date.now();
      if (cachedPath !== location.pathname) {
