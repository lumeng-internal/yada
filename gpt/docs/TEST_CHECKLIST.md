# ChatGPT Yada v2.2.1 测试清单

## Mac mini 本地验证

以下本地验证已于 2026-09-17 在 Mac mini 执行通过；浏览器报告包含 18 组验收。不能当作真实 ChatGPT 页面验收。
...
- [x] `git diff --check`。
- [x] package、lock、manifest、dist manifest、ZIP manifest 均为 2.2.1；ZIP/dist 文件名集合和文件内容逐一相同。
...
- [x] 官方导航出现后 Yada 仍在一个浏览器帧至 100ms 内隐藏。
...
- [ ] 重载 2.2.1 扩展并刷新一篇完整约 18 轮、初始只露出最近若干骨架的 ChatGPT 对话；确认导航立即显示完整轮数，而不是只有已加载骨架。
