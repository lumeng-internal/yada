...
export class RailView {
  ...
  setTurns(turns: readonly YadaTurn[]): void {
    const changed = turns.length !== this.turns.length
      || turns.some((turn, index) => (turn.userMessageId ?? turn.id) !== (this.turns[index]?.userMessageId ?? this.turns[index]?.id));
    this.turns = turns;
    if (!changed) {
  ...
  previewIndex(index: number): void {
    this.hover(index);
  }
